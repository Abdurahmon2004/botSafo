...
class BotController extends Controller
{
    public function webhook()
    {
        ...
        Log::info('Incoming Update: ' . json_encode($update)); // Log incoming update
        if ($update) {
    ...
    {
        Log::info('Callback Query Data: ' . $data); // Log callback query data
        $user = UserWater::where('telegram_id', $chatId)->first();
        if ($data == 'order') {
    ...
    public function start($chatId, $messageId, $user)
    {
        ...
        $btnName = 'keyboard';
        $message = 'Suvga buyurtma berish uchun "📱 Telefon raqamni yuborish" tugmasini bosing 👇 Yoki raqamingizni kiriting (masalan: +998931234567):';
        ...
        if ($contact) {
            ...
            if (preg_match("/^[+][0-9]+$/", $phone) && strlen($phone) == 13) {
                ...
                return Telegram::sendMessage([
                    'chat_id' => $chatId,
                    'text' => 'Sizning raqamingiz mahalliy raqam emas, 📱 bog\'lanish mumkin bo\'lgan raqamingizni yuboring (masalan: +998931234567):',
                ]);
            }
        }
